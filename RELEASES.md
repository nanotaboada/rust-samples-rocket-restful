...
Releases follow the pattern `v{SEMVER}-{NOMINEE}` (e.g., `v1.0.0-benzema`). Codenames are drawn alphabetically from the [Ballon d'Or nominees list](CHANGELOG.md).
...
git checkout -b release/v1.0.0-benzema
```
...
# Example: [1.0.0 - Benzema] - 2026-XX-XX
git add CHANGELOG.md
git commit -m "docs(changelog): prepare release notes for v1.0.0-benzema"
git push origin release/v1.0.0-benzema
```

### 3. Merge the Release PR

Open a pull request from `release/v1.0.0-benzema` into `master` and merge it. The tag must be created **after** the merge so it points to the correct commit on `master`.
...
- [ ] `cargo test` passes
- [ ] Nominee name is valid and follows alphabetical order (see "Ballon d'Or Nominees" in [CHANGELOG.md](CHANGELOG.md))
...
1. Validates tag format (semver and nominee name)
